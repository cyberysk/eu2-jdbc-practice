...
//        Set<Integer> regionIds = new HashSet<>(response.path("items.region_id"));
//
//        assertTrue(regionIds.contains(2) && regionIds.size() == 1);
    ...
    public void test2(){
        Response response = given().accept(ContentType.JSON)
                ...
                .when().get("/employees");
        // we search just IT_PROG so our below assertion is working but what if we delete the line 88
        // response  brings all job_id so contains sometimes does not enough for specific assertion
